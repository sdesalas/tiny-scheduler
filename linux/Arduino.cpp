...
unsigned long pulseIn(uint8_t pin, byte value) { return pulseIn(pin, value, 1000000); }

unsigned long pulseIn(uint8_t pin, byte value, unsigned long usTimeout) {
...
void pinMode(byte pin, byte mode) { printf("Set pin[%d] mode to %c\n", pin, mode); }

void digitalWrite(byte pin, byte value) { printf("Set pin[%d] value to %d\n", pin, value); }
...
void analogWrite(byte pin, byte value) { printf("Set pin[%d] value to %d\n", pin, value); }
...
float Stream::parseFloat() { return parseFloat(NO_SKIP_CHAR); }
